<?php 
///datos de usuario Oncolinux
function usuario_datos_OLX($id,$tipo){
...
WHERE (Usuarios.ID_Usuario = '$id')
 LIMIT 1",$link); 
/* $Usuario_Datos=mysql_query("SELECT clientes.alias, users. *
FROM users, clientes
WHERE (users.id = '$id')
AND users.id_cliente = clientes.id_cliente LIMIT 1",$link); 
*/
$num=mysql_num_rows($Usuario_Datos);
if (mysql_num_rows($Usuario_Datos)!=0){
$i=0;
$id=mysql_result($Usuario_Datos,$i,"ID_Usuario");  
//$id_grupo=mysql_result($Usuario_Datos,$i,"id_grupo");   
$documento_numero=mysql_result($Usuario_Datos,$i,"Identificacion");      
//$nombre_completo=mysql_result($Usuario_Datos,$i,"nombre_completo");      
$p_apellido=mysql_result($Usuario_Datos,$i,"Primer_Apellido");
$s_apellido=mysql_result($Usuario_Datos,$i,"Segundo_Apellido"); 
$s_nombre=mysql_result($Usuario_Datos,$i,"Segundo_Nombre"); 
$p_nombre=mysql_result($Usuario_Datos,$i,"Primer_Nombre");  
$nombre_completo = "$p_apellido $s_apellido $p_nombre  $s_nombre";

$documento_tipo=mysql_result($Usuario_Datos,$i,"Tipo_De_Identificacion"); 
...
$departamento=mysql_result($Usuario_Datos,$i,"Codigo_Del_Departamento_De_Residencia_Habitual"); 
$ciudad=mysql_result($Usuario_Datos,$i,"Codigo_Del_Municipio_De_Residencia_Habitual");      
...
*/
$telefono_fijo=mysql_result($Usuario_Datos,$i,"Telefono");  
$entidad=mysql_result($Usuario_Datos,$i,"EPS");
/*
...
$nuevo_select .="<td><h1>Usuario: </h1><br><h1>$nombre_completo</h1><br>
<h1>Documento: $documento_tipo $documento_numero Codigo: $id</h1><br>

<h1>Fecha impresión: $ahora </h1><br>
<h1>Entidad: $entidad</h1><br>
<h1>Departamento: $nombre_departamento [$departamento] Municipio: $nombre_ciudad [$ciudad] </h1><br>
<h1>Telefonos: $telefono_fijo $telefono_fijo_1 $telefono_celular</h1><br>
</td>";}else {
$nuevo_select .= "
Paciente en consulta: <b>$nombre_completo</b><br>
Documento: <b>$documento_tipo $documento_numero</b> Codigo: <b>$id</b><br>
Fecha nacimiento: <b>$mes_letras $dia  $ano</b> 
Edad: <b>$edad</b> <br>
Entidad: $entidad<br>
Departamento: <b>$nombre_departamento [$departamento] </b>Municipio: <b>$nombre_ciudad [$ciudad] </b>
Barrio: <b>$barrio</b> Telefonos: <b>$telefono_fijo $telefono_fijo_1 $telefono_celular</b><br>
									";
//echo $nuevo_select;
if ($_SESSION['grupo'] == "2"){}
else{
...
$nuevo_select .= "Código del usuario en el sistema: $id";
//return $nuevo_select;
}//fin funcion


/// fin datos de usuario Oncolinux

function usuario_datos_consultar_impresion($id,$tipo,$campo){
global $link;
if($tipo == 'usuario'){$tabla= 'users'; $clave ='id'; $w = "LIMIT 1"; }
...
global $link;
mysql_query("SET NAMES 'utf8'");
$Usuario_Datos=mysql_query("SELECT  users. *
FROM users
WHERE (users.id = '$id')
 LIMIT 1",$link); 
/* $Usuario_Datos=mysql_query("SELECT clientes.alias, users. *
...
<h4>Documento: $documento_tipo $documento_numero Codigo: $id</h4><br>
<h4>Fecha nacimiento: $mes_letras $dia  $ano</h4><br>
<h4>Edad: $edad </h4><br>
<h4>Entidad: $entidad</h4><br>
<h4>Departamento: $nombre_departamento [$departamento] Municipio: $nombre_ciudad [$ciudad] </h4><br>
<h4>Barrio: $barrio Telefonos: $telefono_fijo $telefono_fijo_1 $telefono_celular</h4><br>
</td>";}else {
$nuevo_select .= "
Paciente en consulta: <b>$nombre_completo</b><br>
Documento: <b>$documento_tipo $documento_numero</b> Codigo: <b>$id</b><br>
...
mysql_query("SET NAMES 'utf8'");
$Usuario_Datos=mysql_query("SELECT *
FROM users, especialistas
WHERE (users.id = '$id')
AND users.id = especialistas.id",$link); 
$num=mysql_num_rows($Usuario_Datos);
if (mysql_num_rows($Usuario_Datos)!=0){
$i=0;
$nombre_completo=mysql_result($Usuario_Datos,$i,"nombre_completo");
...
$cargo=mysql_result($Usuario_Datos,$i,"cargo");  
echo "<table border='0' width='100%'><tr><td width='60%'></td><td align='center'><h3>$nombre_completo</h3><br><h2>$cargo $especialidad<br>$universidad_especializacion</h2><br><h4>Registro medico:$registro_medico</h4><br></td></tr></table>";
													}
